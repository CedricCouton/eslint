...
class ConfigArrayFactory {
    ...
    *_normalizeObjectConfigDataBody(
        ...
        const plugins =
            pluginList && this._loadPlugins(pluginList, filePath, name, pluginsRulesPath);
     ...
     * @param {string} pluginsRulesPath the custom path for plugins
     * @returns {Record<string,DependentPlugin>} The loaded parser.
     * @private
     */
    _loadPlugins(names, importerPath, importerName, pluginsRulesPath) {
        return names.reduce((map, name) => {
            if (isFilePath(name)) {
                throw new Error("Plugins array cannot includes file paths.");
            }
            const plugin = this._loadPlugin(name, importerPath, importerName, pluginsRulesPath);
     ...
     * @private
     */
    _loadPlugin(name, importerPath, importerName, pluginsRulesPath) {
        ...
        const relativeTo = path.join(resolvePluginsRelativeTo + (pluginsRulesPath ? `/${pluginsRulesPath}` : ""), "__placeholder__.js");
        ...
        try {
            ...
            if (error && error.code === "MODULE_NOT_FOUND") {
                ...
                error.messageData = {
                    pluginName: request,
                    resolvePluginsRelativeTo: `${relativeTo} / pluginsRulesPath : ${pluginsRulesPath} / importerPath : ${importerPath}`,
                    importerName
                };
            }
        }
